...
ここには設計上の失敗と、**まだ完了していない**修正があります。当初は Sonnet が出した「買い/売り」と「緊急度」を期待リターンに変換し、そのまま view に注入していました。同じLLMの主観を数字に化粧して同じLLMに見直させる循環で、レビューで "confidence laundering" と指摘されたものです。

対応として `bl_alpha_sources.py` に、LLMと独立した3つの alpha 源（アナリスト予想・モメンタム・ファクターベータ）を実装しました。**ただし既定では有効になっていません。** `BL_USE_INDEPENDENT_ALPHA` が既定 `"0"`＝LLM由来 view のみ（後方互換。Ω を大きく取って弱く反映）で、`"1"` で独立源のみ、`"mix"` で併用です。

つまり**現時点の既定経路は、指摘された構造のままです**。独立源への切り替えは用意されているが、既定にはなっていません。
...
第2項は GARCH 推定からの逸脱に罰則をかける正則化です。ただし**過学習を防ぐと断定はできません** — 学習時の VIX とレジームは実データではなく定数（0.2 / 1.0）で渡され、GARCH の σ も銘柄ごとの単一予測値を全期間に敷いています。直近の学習は訓練外評価をしていません。モデルが無ければ GARCH に戻ります。
...
p と b は過去の推奨の採点結果（§11）から銘柄別に集計し、最低5観測を要求します。**ただし呼び出し元はCLIとテストのみで、実際の発注サイズ決定には使われていません。**
...
**現在の判断パイプラインには配線されていません。** 独立したCLIです。またドル円モメンタムは入力されるものの、比率計算に実質使われていません。

具体的な商品コードは、コード側の分類に誤りがあるため本節には載せません。
