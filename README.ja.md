...
ダッシュボードの**システム**画面では、主要な運用機能について「設定上のON/OFF」と「実際に動ける状態」を分け、停止理由とともに表示します。日米の空売り候補レーンは同画面から切り替えられ、実行時の上書きは Git 対象外のローカルな `feature_control_state.json` に保存されます。各空売りレーンでは、設定ユニバース、借株可否proxyのカバー率、直近の価格取得率、検出候補数、借株proxyを通過した候補数を別々に表示します。米国は管理中の広い米国株ユニバースから走査し、証券会社データや価格が欠ける銘柄は安全側に除外します。ONにしても借株データの鮮度・規制・流動性・踏み上げ・インサイダー関門は迂回せず、注文は人間実行のままです。モデル昇格や影実行は検証関門が権威なので、画面では参照のみです。
...
このスナップショットでは、pytest が **3,054件 / `test_*.py` 227ファイル**を収集します。数え方は、件数が`pytest tests/ -q --collect-only`、ファイル数が`find tests -type f -name 'test_*.py'`です。
