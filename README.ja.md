...
ここまでは「AIの提案をどう扱うか」の話でした。ここでは**数字をどう出しているか**を説明します。AIが関与しない、計算だけの部分です。

### どう配分を決めるか

配分計算には [PyPortfolioOpt](https://github.com/robertmartin8/PyPortfolioOpt)・[Riskfolio-Lib](https://riskfolio-lib.readthedocs.io/)・[skfolio](https://skfolio.org/) を使い、目的別に3つの解き方を持っています。

| 手法 | 何を最大化／最小化するか |
|---|---|
| `max_sharpe` | リスク1単位あたりのリターン（シャープレシオ）を最大化 |
| `min_cvar` | 最悪シナリオでの平均損失（CVaR）を最小化 |
| `equal_risk` | 各銘柄のリスク寄与を均等にする |

さらに Black-Litterman を使い、市場が織り込んでいる期待リターンに自分の見通しを混ぜます。

**ここに設計上の失敗と、その修正があります。** 当初は Sonnet が出した「買い/売り」と「緊急度」を期待リターンに変換して、そのまま Black-Litterman の view に入れていました。これは**同じLLMの主観をいったん数字に化粧して、同じLLMに見直させる循環**でしかなく、独立した予測になっていません。レビューで "confidence laundering"（確信度のロンダリング）と指摘されました。

現在は `bl_alpha_sources.py` に、LLMと独立した3つの alpha 源を用意しています。

- `analyst_consensus_alpha` — 外部アナリスト予想
- `momentum_alpha` — 価格モメンタム
- `factor_beta_alpha` — ファクターベータ

### リスクをどう測るか

**Cornish-Fisher VaR / CVaR** を使います。通常のVaRはリターンが正規分布だと仮定しますが、実際の相場は「たまに極端に動く」ため裾が厚くなります。Cornish-Fisher は歪度と尖度で補正し、その厚みを織り込みます。

ボラティリティ（値動きの荒さ）の予測は **GJR-GARCH** です。「下落局面のほうが荒れやすい」という非対称性を扱えます。

さらに `ginn_model.py` で **GINN（GARCH-Informed Neural Network, ICAIF 2024）** を実装しています。GJR-GARCH の予測を LSTM の制約として使う構成です。

```
入力: [リターン, σ_GARCH, VIX, レジーム] × 60日
モデル: 2層LSTM (hidden=64) + 線形出力
損失: MSE(σ予測, |実際の変動|) + 0.3 × MSE(σ予測, σ_GARCH)
```

第2項が効きます。ニューラルネットが自由に予測すると過学習しますが、GARCH の推定から離れすぎると罰則がかかるので、統計モデルに引き戻されます。

**リスク計算には現在の保有構成を使います。** 自分のNAV履歴は短く、過去の会計バグで汚れてもいます。そこで `portfolio_risk_returns.py` が、**いまの保有ウェイトを過去の市場価格に当てて**日次リターンを再構成します。

```
ポートフォリオリターン_t = Σ w_i × r_i,t     w_i = 現在の時価ウェイト
```

これで数百営業日ぶんの、会計バグの影響を受けないリターン系列が得られます。

**リスクモデル自体も検証します。** `risk_model_validation.py` が日々のVaR予測を保存し、**Kupiec の POF検定**にかけます。「95% VaR を超えた日が、本当に約5%に収まっているか」を統計的に検定するもので、外れが多すぎても少なすぎても、モデルが現実と合っていないと分かります。

### どれだけ買うか

`kelly_sizing.py` が **half-Kelly** でサイズを提案します。

```
kelly = 0.5 × (p×b − q) / b
  p = 勝率, q = 1−p, b = 平均利益 ÷ 平均損失
```

勝率と損益比は、過去の推奨を実際に採点した結果（§11）から銘柄別に集計します。フルKellyは理論上の最大成長率ですが実際には振れが大きすぎるため、半分に落としています。

### いつ買い増すか

`drawdown_dca_engine.py` は「下げ切る少し前から拾い始めないと、反転時に取り逃す」を仕組みにしたものです。3つのトランシェ（T1/T2/T3）が別々の条件で発動します。

| | 主導する指標 |
|---|---|
| T1 | VIXのピークからの減衰 |
| T2 | Fear & Greed 指数 + ハイイールド債スプレッド（500bps以上） |
| T3 | 上記の複合 |

単一指標では騙されるので、複数条件のANDで判定します。

### 為替をどうするか

`fx_hedge_manager.py` が、レジーム × VIX × ドル円モメンタム × インプライドボラティリティから目標ヘッジ比率を出します。固定比率にせず **0〜70%** の範囲で動かし、日々の変更幅は **±10%** に制限します（往復ビンタを避けるため）。

手段は3つ — 為替ヘッジ付きETF（1655.T / 2040.T）、先物、単純な円の積み増しです。

### 税金をどう扱うか

`tax_lot.py` が、イベント台帳の売買履歴から**銘柄ごとの取得ロット（いつ・いくらで買った分か）をFIFOで再構成**します。

これが必要な理由があります。保有ファイルは加重平均単価しか持っていないので、「含み損の出ている特定のロットだけを売って損を確定する」「NISA枠を踏まないロットを選ぶ」という制御ができませんでした。ロット単位で追えると、狙った税務効果を出せます。

`tax_optimizer.py` は損出し候補の検出、NISA残枠、外国税額控除のシミュレーションを担当します。

### 成績を分解する

`factor_attribution.py` が、月次リターンを ETF で代用したファクターに **OLS回帰**して α・β・R² を出します。
...
「儲かったのは自分の腕（α）なのか、単に市場やスタイルに乗っていただけ（β）なのか」を切り分けるためです。日本から買えるETFだけで組んでいます。

### レジーム判定とパラメータ

相場の局面は HMM（隠れマルコフモデル）で判定し、局面ごとに違うパラメータを使います。この値は `backtest_wfo.py` の**ウォークフォワード最適化**（期間を区切って「学習→次の期間で検証」を繰り返す方法）から生成しています。

**ただし限界も書いておきます。** 強気レジームは過剰適合を避けるため全期間統合のパラメータを使っており、**弱気レジームはサンプル数が足りず中立レジームの値を流用**しています。局面別に最適化しきれてはいません。

### その他

- `technical_signals.py` — RSI・MACD・ボリンジャーバンド・出来高を保有銘柄と市場ETFに対して計算
- `rebalance_engine.py` — 通貨配分・セクター配分の逸脱を検出し、優先順位を付けた売買指示を出す
- `pair_screener.py` — ロング・ショートのペア候補
...
このスナップショットでは、pytest が **2,706件 / 202ファイル**を収集します（正確な現在値は `pytest tests/ -q --collect-only` で確認できます）。

件数より中身のほうが重要で、**守るべき不変条件をファイル名に持つテストが13本**あります（safety / gating / policy / guard / integrity / privacy）。たとえば次のようなものです。
