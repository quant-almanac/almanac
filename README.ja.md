...
そして現在、**この自動適用は無効にしてあります**（`tuning_auto_mode.json` の `enabled: false`）。2026年7月のレビューで、古いログを根拠にした推奨をそのまま適用していたことが分かったためです。状態・履歴・ログの監査と許可リストの見直しが済むまで、定期実行は動いても何も適用しません。理由もファイルに書き残してあります。

この「作ったが、信用できないので止めてある」という状態を隠さないこと自体が、このシステムの方針です。
