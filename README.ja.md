...
前節までの「一日の流れ」は、**すでに保有している銘柄をどうするか**が中心です。新しい候補を探すのは別の仕組みで、対象と時間軸によって2つに分かれています。

**短期・スイング候補（`screener.py`）**
...
こちらは週次（日曜の朝）です。投資テーゼの文章生成には Batch API を使い、結果を待つ代わりに料金が半額になります。急がない処理なので、遅くて安いほうを選んでいます。
