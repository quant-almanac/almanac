...
ダッシュボードの**システム**画面では、主要な運用機能について「設定上のON/OFF」と「実際に動ける状態」を分け、停止理由とともに表示します。日米の空売り候補レーンは同画面から切り替えられ、実行時の上書きは Git 対象外のローカルな `feature_control_state.json` に保存されます。ONにしても借株データの鮮度・規制・流動性・踏み上げ・インサイダー関門は迂回せず、注文は人間実行のままです。モデル昇格や影実行は検証関門が権威なので、画面では参照のみです。
