...
**まず情報を最新にする。** 関門が判断に使う材料 — 経済イベントの予定、テクニカル指標、VIX（後述）、決算の近さ、相場シナリオ — は、分析を始める**前**に鮮度を確認し、拒否期限より前に作り直します。更新期限とstale期限は1つのregistryで管理し、CIが`refresh < stale`を要求します（technical 4/8時間、macro 12/24時間、news 6/12時間、options 12/24時間）。これにより、日次producerが更新をskipした直後にconsumerが同じ値をstaleとして拒否する境界事故を防ぎます。更新失敗は隠さず、macro fallbackとoptions入力の状態はwatchdogにも渡し、calendar欠落は「問題なし」でなく`review`にします。
...
この関門では数量と金額を分けて扱います。株・口数の`amount_hint`は数量だけ、`estimated_notional_jpy`が金額の数値権威で、ダッシュボードの組み合わせ表示はresponse生成時に作ります。決定論的exit sizingがLLM原案の数量を置き換えた場合、model confidenceは方向だけへのconfidenceと明記し、機械計算した数量へのconfidenceとして表示しません。
