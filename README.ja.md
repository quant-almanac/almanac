...
> **移植性の範囲:** 「はじめかた」に書いた経路と、ここで案内するスクリーナーの定期実行は、clone先を基準に状態ファイルを読み書きします。一方、古い補助ツールの一部（旧Telegram／帳票スクリプト、静的ダッシュボード生成、バックテスト補助など）には、参考運用環境の名前 `~/portfolio-bot` が残っています。別名のcloneから、本文で案内していないトップレベルのスクリプトを使う場合は、先に入出力先を確認してください。
...
そのあと Claude Opus が最終的なとりまとめを行います。通常は指定したツールを強制し、その構造化入力を読みます。ベンダーが可視テキストを返した場合にJSONを取り出す互換経路もあり、その後に空結果や行動項目の機械的な検査を重ねます。また、**出力の途中で文字数上限に達して切れた応答は、たとえ中身が入っていても採用せず捨てます**。中途半端な答えを「そういう結論だった」と読み違えないためです。
...
肝は、抽出した特徴が最初は **observe_only（観察のみ）** として入ることです。昇格判定のスクリプトは、開示の種類ごとにその後の超過収益と突き合わせますが、作るのはガバナンス用の判定案だけです。設定を勝手に書き換えたり、生の特徴量をそのまま発注候補に変えたりはしません。

一方で、観察中の材料を小さく試すための、別の限定経路があります。最終とりまとめが出所・理由・暫定判断の印を明示し、後段の機械処理が確信度の下限と金額上限を確認した場合にだけ、暫定的な提案として通常の関門へ渡せます。生の `observe_only=true` の提案は却下されます。つまり「成績を測る」「正式な昇格を検討する」「上限つきで暫定判断する」は、別々の仕組みです。
...
要するに**ふるいの構造**です。安いモデルが全部に目を通し、高いモデルは生き残ったものだけを見る。主要な分析・スクリーニング・開示処理・監視の通信経路は、トークン使用量と推定コストを共通のLLM呼び出しログへ記録するので、計装済みの経路については費用を実測できます。
...
| `var_budget` | 想定される1日の損失（VaR、95%水準）が相場別の上限以上なら、新規の買いを**すべて**却下。上限は弱気・ストレス時1.2%、通常時1.6%、強気確認済みかつVIX<25なら2.0%。環境変数で変えても既定の絶対上限は2.3% |
...
公開情報・匿名化情報だけを使う呼び出し（スクリーニング、開示情報の特徴量抽出）は、この設定の影響を受けません。設計上、その送信内容にポートフォリオの中身を含めない経路だからです。共通の検査関門を通るものと、レビュー済みの公開専用スクリーナーとしてベンダーへ直接つなぐものがあります。テストは直接接続を持つファイルに関門の印または許可リスト登録があるかを検査し、重要経路には個別テストもありますが、ファイル単位の検査だけで全呼び出しの情報の流れを証明するものではありません。
...
`holdings.json`・`account.json`・`nisa_portfolio.json`・`trade_history.csv`・`almanac.db` などは Git の管理対象外なので、通常のコミットでは公開されません。ただし Git ignore は情報流出を防ぐ境界ではありません。設定で許可した book-aware のAI呼び出しは必要な保有情報を外部へ送りますし、利用者が無視対象のファイルを手作業でアップロードすることもできます。ドキュメント中の金額例は実際の数字ではなく、丸めた仮の値です。
...
`launchagents/` と [`examples/crontab.example`](examples/crontab.example) は、セットアップ時に自動登録されません。macOSで使う場合は、選んだ plist の `/path/to/ALMANAC` をすべて自分の場所に直し、実行コマンドとログ出力先を確認してから、必要なものだけを自分の `~/Library/LaunchAgents` にコピーして読み込んでください。スクリーナーの周期を使う場合は、crontab例の `ALMANAC_DIR` を自分のclone先に直したうえで、必要な行だけを `crontab -e` へ入れます。どちらも日本時間、`venv/` と `~/.almanac_secrets` の作成済みを前提にしています。
