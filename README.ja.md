...
> **どこに置いても動くか:** この文書で案内している経路（「はじめかた」と定期実行）は、clone した場所を基準に動きます。
>
> ただし、本文で案内していないトップレベルのスクリプトの一部には、開発環境のパス `~/portfolio-bot` が残っています。影響は2種類です。**コメント内の実行例が古いだけ**のものは無害ですが、`analyzer.py` や `start.sh` のように**読み書き先が固定されている**ものもあり、別の場所へ clone するとそちらを触ります。
>
> 使う前に確認してください。
>
> ```bash
