...
公開版の初期状態は「全商品OFF」ではありません。**計算・観測を常時動かすこと**と、**実際の提案を変更する権限**を分けています。現物株・非レバレッジETF・投信は稼働、信用買い・空売り候補は安全関門を満たす場合だけ、オプションは分析シグナルだけ、half-Kelly と FX ヘッジは影実行、税務取得費は新旧比較です。GINN は将来更新として残しますが、現在候補は判断から除外し、検証済みbundleが昇格しない限り GJR-GARCH を権威とします。execution-plan の関門は observe から始まり、どのモードにも証券会社への自動発注権限はありません。完全な有効化契約は [`objective.md`](objective.md#8-機能商品の有効化契約) にあります。
...
この構成では、ALMANACへ読み込んだ確認済み現金をすべて余剰投資資金として扱い、システム内の生活防衛用現金は0円です。ただし相場に応じて戦術的に現金を残すことはあり、保護額を0円にしても口座・鮮度・発注意図・受渡・税・手数料・必要保証金を確認する既存関門は無効になりません。実行計画が配備できるのは戦術現金目標を超える分だけで、その余剰を強い強気／弱い強気／中立／弱い弱気では2／3／6／12か月に分けます。強い弱気またはshock中は通常買付枠を作りません。当月の約定は枠を消費しますが、約定後の減った現金から二重に枠を縮めず、確認済みの入出金は自動で再計算に反映します。現金の権威は経過時間ではなくイベントで管理し、時間が過ぎただけでは確認済み残高を失効させませんが、後続の未照合口座イベントがあれば再確認が必要です。家計の生活防衛資金は、このシステムが表す投資口座の外で管理する前提です。
...
**まず情報を最新にする。** 定期更新する判断入力（テクニカル、経済イベント、
ニュース、オプション）は1つの閾値registryを共有し、CIが`refresh < stale`を
要求します（technical 4/8時間、macro 12/24時間、news 6/12時間、options
12/24時間）。保有・現金・為替・スクリーニングもregistryのstale期限を使いますが、
更新権威はそれぞれの証跡・イベント経路です。VIX、決算の近さ、相場シナリオは
分析入口に専用検査がありますが、この定期更新registryの保証対象ではありません。
不正な更新閾値overrideは表示だけして続行せず、cache利用前に分析を停止します。
calendarや必要なoptionsの欠落・staleはactionを`review`にし、optionsは要求銘柄の
分母を保持するため、取得障害が`0件中0件=正常`にはなりません。時刻上freshでも
fallback利用で`degraded`のmacroはwatchdogへ警告しますが、それだけで全actionを
止める設計ではありません。欠落またはstaleなら止めます。
...
費用は実測できます。分析・スクリーニング・開示処理・監視といった主要な経路が、トークン使用量と推定コストを共通のログへ書き出すためです。日次分析はdata gatherより前に`analysis_id`を発行して並列workerへ伝播するため、1実行のcostは時刻範囲で推測せずIDでjoinできます。ただし記録されるのは計装済みの経路だけで、支出のすべてを捕捉しているわけではありません。
...
この関門では数量と金額を分けて扱います。株・口数の`amount_hint`は数量だけ、
`estimated_notional_jpy`が金額の数値権威です。APIがこの2つから表示を生成し、
数値権威が無いときにclientが旧説明文の埋込金額を復活させてはいけません。
決定論的exit sizingがLLM原案の数量を置き換えた場合、model confidenceは方向
だけへのconfidenceと明記し、機械計算した数量へのconfidenceとして表示しません。
...
| **将来更新** | 着想はロードマップに残すが、新しい検証契約を通るまで現実装を判断から外す |
| **retired** | 通常運用では使わない |
...
**GINN（研究用・将来更新）** — `ginn_model.py` に実験的な GINN-inspired モデルがあります。ただし [ICAIF 2024 の GINN 原論文](https://arxiv.org/abs/2410.00288)を忠実に再現したものではありません。
...
この差は重要です。原論文は90日入力、256ユニットのLSTMを3層、300 epoch、分散を対象にした結合損失を報告し、GARCH側の重みも別データで調整しています。現在コードは60日入力、64ユニットを2層、50 epoch、翌日の絶対リターンを目標にし、別の形の`0.3` GARCH罰則を使います。原論文に指数データで有望な結果があっても、構造も学習対象も違う小型実装を個別ポートフォリオの銘柄群で有効だと証明するものではありません。

本番利用は fail-closed（安全を確認できなければ使わない）です。検証メタデータのない旧モデルや、事前に固定した検証基準を満たさない候補は拒否し、GJR-GARCH に戻します。tier成果物・Today API・ダッシュボードへ渡すモデル名も、固定の「GINN」ではなく実際に使ったモデルです。ダッシュボードは稼働modelと最新の監査候補を分離し、候補manifestからvalidation指標を読みます。forward fieldは現在schema予約だけで、forward評価pipelineは未実装です。保存値0を実績として表示しません。すぐ古くなる実測値はREADMEへ固定しません。この安全ゲートは研究モデルの妥当性を証明するものではなく、未検証モデルを判断へ混入させないためのものです。

学習は、学習区間を広げていく3つのrolling-origin foldで検証します。各foldでは、その時点の学習区間だけで正規化統計とGARCH基準を作り、直後のvalidation区間を評価します。検証後は利用可能な履歴全体で候補を再学習します。version付きbundleには`model.pt`・manifest・推論で使う銘柄別scalerを保存し、それぞれchecksumで検証します。昇格には、事前固定したvalidation件数・銘柄数・特徴量充足率・データ鮮度・GARCH比の誤差基準をすべて満たす必要があり、推論時にも明示的な銘柄経路と一致する保存済みscalerを要求します。walk-forward validationを「最後まで触れていないtest」と表現しません。昇格後の未観測性能は、別のforward evaluatorを実装した後に初めて測ります。

したがって、これは削除ではなく延期です。将来版では、原論文の契約に合わせるか、差を明示して **GINN-inspired** と呼びます。定数VIX・レジームと単一GARCH入力を、look-aheadのない履歴系列とrolling GARCH σへ替え、学習と推論の変換を一致させ、その後の未観測期間をshadowで測ります。判断経路へ戻す予定条件は、GJR-GARCHよりMSEとQLIKEがともに5%以上改善し、tail・peak特性を悪化させず、3倍ボラティリティclampを頻発せず、最低60営業日のforward実績があることです。合格しても最初は影響上限付きcanaryで、自動注文権限は与えません。
...
| **GINN** | 実際の値動きとGARCH推定を組み合わせる研究用ニューラルネット。現在のALMANAC実装は小型のGINN-inspired版で判断から除外中。将来のpaper-aligned版または差を明示した版がwalk-forward・保存済みscaler・shadow-forwardの昇格契約を通った場合だけ判断へ入れる |
