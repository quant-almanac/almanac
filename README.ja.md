...
証券会社で確認した保有数量・現金・NISA枠・ヘッジ残高は、**72時間ごとの期限ではなくイベント駆動**で有効性を判定します。時間が経っただけでは株数や取得原価は変わりません。snapshot後に既知の約定があれば、影響するポジションまたは買付余力を再照合まで無効にし、時刻や口座経路が曖昧なら安全側に倒します。ALMANAC外で行った売買・入出金・移管は、CSV取込または記録が必要です。観測できない外部イベントをシステムだけで検知することはできません。価格・VIX・ニュースなどの市場データには、これとは別に従来どおり時間ベースの鮮度期限があります。
