...
def _build_execution_plan_view(plan: dict, board: list[dict], synthesis: dict, now: datetime) -> dict:
    ...
    return {
        ...
        "budgets": {
            ...
            "deployment_basis_surplus_jpy": budgets.get("deployment_basis_surplus_jpy"),
            "deployment_basis_filled_buys_added_back_jpy": budgets.get("deployment_basis_filled_buys_added_back_jpy"),
            "deployment_basis_sell_proceeds_removed_jpy": budgets.get("deployment_basis_sell_proceeds_removed_jpy"),
            "deployment_basis_method": budgets.get("deployment_basis_method"),
            "deployment_months": budgets.get("deployment_months"),
            "deployment_regime_level": budgets.get("deployment_regime_level"),
            "deployment_regime_label": budgets.get("deployment_regime_label"),
            "deployment_policy_version": budgets.get("deployment_policy_version"),
            "ordinary_deployment_allowed": budgets.get("ordinary_deployment_allowed"),
...
def _build_today() -> dict:
    ...
    def _board_row(a: dict, *, force_review: bool = False) -> dict:
        ...
        # Stage -1: 無効化された分析/action_state はここで review へ強制する。
        ...
        try:
            ...
            inv = resolve_execution_invalidation(
                ...
                action_state_id=(entry or {}).get("id"),
            )
        except Exception:
            inv = None
        if inv is not None:
            readiness = "review"
            block_reasons = [invalidation_block_reason(inv)] + block_reasons
